...
trirel_s <- FreqR_Frase(as.matrix(df3_s[1:l,1]),dados_s)
...
tetrarel_s <- FreqR_Frase(as.matrix(df4_s[1:l,1]),dados_s)
...
pentarel_s <- FreqR_Frase(as.matrix(df5_s[1:l,1]),dados_s)
...
trirel_j <- FreqR_Frase(as.matrix(df3_j[1:l,1]),dados_j)
...
tetrarel_j <- FreqR_Frase(as.matrix(df4_j[1:l,1]),dados_j)
...
pentarel_j <- FreqR_Frase(as.matrix(df5_j[1:l,1]),dados_j)
pentarel_j <- pentarel_j[head(order(pentarel_j[,2],decreasing = TRUE),dim(pentarel_j)[1]),]
```

Os dados obtidos da platafora *TripAdvisor* resultaram em `r nrow(data_s)` comentários para Superagui e `r nrow(data_j)` para o Jalapão. As frequências de ocorrência dos 15 primeiros unigramas podem ser visualizados na Tabela \ref{tab1}. As palavras mais comuns de Superagui foram "`r df_s[1,1]`" e "`r df_s[2,1]`" com `r df_s[1,2]` e `r df_s[2,2]` ocorrências respectivamente. Já para o Jalapão, "`r df_j[1,1]`" e "`r df_j[3,1]`" foram as mais frequentes, com `r df_j[1,2 ]` e `r df_j[3, 2]` ocorrências, respectivamente. É possivel observar que a palavra "não" apareceu entre as 3 palavras mais citadas nos dois atrativos turísticos. Portanto, convém observar quais são as palavras subsequentes da negação para compreender melhor o contexto.


```{r tab1,  message=FALSE, warning=FALSE}

unigram<-cbind(df_s[1:l,], round(relativa_s$FreqR[1:l], 2), df_j[1:l,], round(relativa_j$FreqR[1:l], 2))
colnames(unigram) <- c("Superagui", "FA", "FR", "Jalapão", "FA", "FR")


knitr::kable(unigram[1:15,], "latex", booktabs = TRUE, align = "l",
...
bigram<-cbind(df2_s[1:l,], round(birel_s$FreqR[1:l], 2), df2_j[1:l,], round(birel_j$FreqR[1:l], 2))
...
bigram_n<-cbind(nao_bigram_s[1:l,], nao_bigram_j[1:l,])
colnames(bigram_n) <- c("Superagui", "FA", "Jalapão", "FA")
```

Sendo assim, duas tabelas de bigramas foram geradas. Uma com todos os bigramas do *corpus* (Tabela \ref{tab2}) e outra com os bigramas que iniciam com a palavra "não" (Tabela \ref{tab3}). Pode-se perceber que os bigramas negativos de Superagui "`r nao_bigram_s[1,1]`" e "`r nao_bigram_s[2,1]`" sugerem algum tipo de alerta, enquanto que os do Jalapão "`r nao_bigram_j[2,1]`", "`r nao_bigram_j[3,1]`" e "`r nao_bigram_j[4,1]`" correspondem a um tipo de relato, com certa expressão de frustação, o que já demonstra um tipo de indício da experiência turística do lado ativo.


```{r tab2,  message=FALSE, warning=FALSE,  fig.align="center"}

knitr::kable(bigram[1:15,],"latex", booktabs = TRUE, align = "l",
      caption = "\\label{tab2}Frequência de bigramas") %>%
...
```{r tab3,  message=FALSE, warning=FALSE,  fig.align="center"}

knitr::kable(bigram_n[1:15,],"latex", booktabs = TRUE, align = "l",
      caption = "\\label{tab3}Frequência de bigramas de negação") %>%
      kable_styling(latex_options = "hold_position") %>%
      row_spec(0, bold = TRUE) 
...
trigram<-cbind(df3_s[1:l,], round(trirel_s$FreqR[1:l], 2), df3_j[1:l,], round(trirel_j$FreqR[1:l], 2))
colnames(trigram) <- c("Superagui", "FA", "FR", "Jalapão", "FA", "FR")
```


Os bigramas mais comuns de Superagui foram `r df2_s[1,1]` (`r df2_s[1,2]`) e `r df2_s[2,1]` (`r df2_s[2,2]`). Já para o Jalapão, `r df2_j[1,1]` (`r df2_j[1,2]`) e `r df2_j[2,1]` (`r df2_j[2, 2]`) foram os mais frequentes (Tabela \ref{tab3}).




```{r tab5,  message=FALSE, warning=FALSE,  fig.align="center"}

tetragram<-cbind(df4_s[1:l,], round(tetrarel_s$FreqR[1:l], 2), df4_j[1:l,], round(tetrarel_j$FreqR[1:l], 2))
colnames(tetragram) <- c("Superagui", "FA", "FR", "Jalapão", "FA", "FR")

knitr::kable(tetragram[1:15,],"latex", booktabs = TRUE, align = "l",
      caption = "\\label{tab5}Frequência de tetragramas") %>%
      kable_styling(latex_options = "hold_position") %>%
      row_spec(0, bold = TRUE) 
```
...
```{r fig3, fig.cap="Wordcloud de bigramas", echo = FALSE, fig.show='hold'}


par(mfrow=c(1, 2))
wordcloud(words = df2_s$ngram,
                freq = df2_s$n,
                max.words = 80,
                scale = c(3,0.2),
                colors = brewer.pal(8, "Dark2"), 
...
```{r fig4, fig.cap= "Frequência td-idf dos atrativos Jalapão e Superagui"}
...
atrativo_tf_idf %>%
  ...
  ungroup() %>%
  ggplot(aes(tf_idf, fct_reorder(word, tf_idf), fill = V1)) +
...
````{r , echo=FALSE, warning=FALSE}
datad<- read.csv2("../01_Dados/00_Comentariosjalapao.csv", encoding = 'UTF-8')

datad<-datad[,2:3]

datad_c<-clean_text(datad$coments)
datad<- as.data.frame(cbind(datad$atrativo, datad_c))
atrativo_words_d <- datad %>%
  unnest_tokens(word, datad_c) %>%
  count(V1, word, sort = TRUE)

total_words_d <- atrativo_words_d %>% 
  group_by(V1) %>% 
  summarize(total = sum(n))

atrativo_words_d <- left_join(atrativo_words_d, total_words_d)

atrativo_tf_idf_d <- atrativo_words_d %>%
  bind_tf_idf(word, V1, n)
...
```{r fig5, fig.cap= "Frequência td-idf dos atrativos do Jalapão"}
...
```{r fig8, fig.cap="Análise de sentimentos de Superagui", message=FALSE, warning=FALSE,  fig.align="center"}

dado_s<-tibble(dados_s)
sentimentos_s <- dado_s %>%
  unnest_tokens(ngram, dados_s, drop = FALSE) %>%
  mutate(polaridade = unlist(get_polaridade_vec(ngram)))

token_sentiments_s <- sentimentos_s %>%
  mutate(polaridade = unlist(get_polaridade_vec(sentimentos_s$ngram))) %>%
  mutate(sentimento = factor(polaridade, levels = c(-1,0,1), labels = c("Negativo", "Neutro", "Positivo")))

token_sentiments_counts_s <- token_sentiments_s %>% count(ngram, polaridade, sentimento, sort = TRUE)

token_sentiments_counts_s %>%
  filter(polaridade != 0) %>%
...
```{r fig9, fig.cap="Análise de sentimentos do Jalapão", message=FALSE, warning=FALSE,  fig.align="center"}

dado_j<-tibble(dados_j)
sentimentos <- dado_j %>%
  unnest_tokens(ngram, dados_j, drop = FALSE) %>%
  mutate(polaridade = unlist(get_polaridade_vec(ngram)))
...
```{r fig10,  fig.cap="Dendrograma de Superagui", message=FALSE, warning=FALSE,  fig.align="center"}

myDtm_s = dtm(dados_s)
dtms_s <- removeSparseTerms(myDtm_s, 0.88)
dtms_matrix_s <- as.matrix(dtms_s)
dist_s <- dist(t(dtms_matrix_s), method="euclidean")
fit_comentarios_s <- hclust(d=dist_s, method="complete")
dend_comentarios_s <- color_branches(fit_comentarios_s, k = 8)
par( mar = c(4,4,4,2)+2.5)
plot(dend_comentarios_s, yaxt='n')
```




```{r fig11,  fig.cap="Dendrograma do Jalapão", message=FALSE, warning=FALSE,  fig.align="center"}

myDtm_j = dtm(dados_j)
dtms_j <- removeSparseTerms(myDtm_j, 0.88)
dtms_matrix_j <- as.matrix(dtms_j)
dist_j <- dist(t(dtms_matrix_j), method="euclidean")
fit_comentarios_j <- hclust(d=dist_j, method="complete")
dend_comentarios_j <- color_branches(fit_comentarios_j, k = 8)
par( mar = c(4,4,4,2)+2.5)
plot(dend_comentarios_j, yaxt='n')
```
...
review_bigrams_s <- tibble(dados_s) %>%
  unnest_tokens(ngram, dados_s, token = "ngrams", n = 2)%>%
  separate(ngram, c("word1", "word2"), sep = " ") %>%
  count(word1, word2, sort = TRUE)

review_bigrams_j <- tibble(dados_j) %>%
  unnest_tokens(ngram, dados_j, token = "ngrams", n = 2)%>%
  separate(ngram, c("word1", "word2"), sep = " ") %>%
  count(word1, word2, sort = TRUE)
```
...
``` {r fig6, fig.cap="Redes de bigramas de Superagui", echo = FALSE}
...
``` {r fig7, fig.cap="Redes de bigramas do Jalapão", echo = FALSE}
...
```{r fig12,  fig.cap="Análise de tópicos do Jalapão", message=FALSE, warning=FALSE,  fig.align="center"}



ap_topics1_j<-ap_topics(df_j)
ap_top_terms1_j<-ap_top_terms(ap_topics1_j)
ap_top_terms1_j %>%
...
```{r fig13,  fig.cap="Análise de tópicos do Jalapão", message=FALSE, warning=FALSE,  fig.align="center"}

beta_spread_j <- ap_topics1_j %>%
